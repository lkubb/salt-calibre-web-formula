...
We will see a quick setup to configure the ``calibre_web`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``calibre_web`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``calibre_web`` formulas under ``/srv/salt/calibre_web/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/calibre_web/parameters/dns:domain/
    ln -s dns:domain /srv/salt/calibre_web/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/calibre_web/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/calibre_web/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``calibre_web`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``calibre_web`` formulas under ``/srv/salt/calibre_web/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/calibre_web/parameters/roles
...
We create a configuration for the role ``calibre_web/server`` in ``/srv/salt/calibre_web/parameters/roles/calibre_web/server.yaml``:
...
We create another configuration for the role ``calibre_web/client`` in ``/srv/salt/calibre_web/parameters/roles/calibre_web/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles calibre_web/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles calibre_web/client
...
.. code-block:: console
    ...
    mv /srv/salt/calibre_web/parameters/dns:domain/  '/srv/salt/calibre_web/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-calibre_web-as-json:
      test.nop:
        - name: {{ calibre_web | json }}
...
Here is an example based on `template-formula/calibre_web/config/file.sls`_:
...
.. code-block:: sls
    ...
    calibre_web-config-file-file-managed:
      file.managed:
        - name: {{ calibre_web.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='calibre_web-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ calibre_web.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``calibre_web`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ calibre_web['winner'] }}
...
.. _template-formula/calibre_web/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/calibre_web/config/file.sls
